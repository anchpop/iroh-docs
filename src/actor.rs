...
impl SyncHandle {
    ...
    pub async fn default_author(&self) -> Result<Option<AuthorId>> {
        let (reply, rx) = oneshot::channel();
        self.send(Action::DefaultAuthor { reply }).await?;
        rx.await?
    }

    pub async fn initialize_default_author(&self, author: Author) -> Result<AuthorId> {
        let (reply, rx) = oneshot::channel();
    ...
    pub async fn set_default_author(&self, author: AuthorId) -> Result<()> {
        let (reply, rx) = oneshot::channel();
